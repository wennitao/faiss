...
__global__ void multiHeadIvfInterleavedScan2(
    ...
    if constexpr ((NumWarpQ == 1 && NumThreadQ == 1) || NumWarpQ >= kWarpSize) {
        ...
        for (; i < limit; i += blockDim.x) {
            ...
            // The IDs reported from the list may be -1, if a particular IVF
            // list doesn't even have k entries in it
            if (listIds[headId][queryId][curProbe] != -1) {
                // Adjust the value we are selecting based on the sorting order
                heap.addThreadQ(distanceBase[i] * adj, index);
            }
        ...
        if (i < num) {
            ...
            idx_t listId = listIds[headId][queryId][curProbe];
            if (listId != -1) {
                heap.addThreadQ(distanceBase[i] * adj, index);
            }
        }
        ...
        for (auto i = threadIdx.x; i < k; i += blockDim.x) {
            // Re-adjust the value we are selecting based on the sorting order
            distanceOut[headId][queryId][i] = smemK[i] * adj;
            auto packedIndex = smemV[i];
            ...
            indicesOut[headId][queryId][i] = index;
        }
    }
}
...
multiHeadIvfInterleavedScan2<THREADS, NUM_WARP_Q, NUM_THREAD_Q>   \
    ...
    if (k == 1) {
        MULTI_HEAD_IVF_SCAN_2(128, 1, 1);
    } else if (k <= 32 && getWarpSizeCurrentDevice() == 32) {
        MULTI_HEAD_IVF_SCAN_2(128, 32, 2);
    } else if (k <= 64) {
        MULTI_HEAD_IVF_SCAN_2(128, 64, 3);
    } else if (k <= 128) {
        MULTI_HEAD_IVF_SCAN_2(128, 128, 3);
    } else if (k <= 256) {
        MULTI_HEAD_IVF_SCAN_2(128, 256, 4);
    } else if (k <= 512) {
        MULTI_HEAD_IVF_SCAN_2(128, 512, 8);
    } else if (k <= 1024) {
        MULTI_HEAD_IVF_SCAN_2(128, 1024, 8);
    }
    #if GPU_MAX_SELECTION_K >= 2048
    else if (k <= 2048) {
...
void runMultiHeadIVFInterleavedScan(
    ...
    if (k == 1) {
        ivf_interleaved_call(multiHeadIvfInterleavedScanImpl<128, 1, 1>);
    } else if (k <= 32 && getWarpSizeCurrentDevice() == 32) {
        ivf_interleaved_call(multiHeadIvfInterleavedScanImpl<128, 32, 2>);
    } else if (k <= 64) {
        ivf_interleaved_call(multiHeadIvfInterleavedScanImpl<128, 64, 3>);
    } else if (k <= 128) {
        ivf_interleaved_call(multiHeadIvfInterleavedScanImpl<128, 128, 3>);
    } else if (k <= 256) {
        ivf_interleaved_call(multiHeadIvfInterleavedScanImpl<128, 256, 4>);
    } else if (k <= 512) {
        ivf_interleaved_call(multiHeadIvfInterleavedScanImpl<128, 512, 8>);
    } else if (k <= 1024) {
        ivf_interleaved_call(multiHeadIvfInterleavedScanImpl<128, 1024, 8>);
    }
    #if GPU_MAX_SELECTION_K >= 2048
    else if (k <= 2048) {
        ivf_interleaved_call(multiHeadIvfInterleavedScanImpl<64, 2048, 8>);
    }
    #endif
}
