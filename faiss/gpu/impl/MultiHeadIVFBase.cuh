...
/// MultiHead Base inverted list functionality for IVFFlat and IVFPQ
class MultiHeadIVFBase {
public:
    ...
    idx_t getNumHeads() const;

    /// Returns the number of inverted lists
    idx_t getNumLists(idx_t headId) const;

    /// For debugging purposes, return the list length of a particular
    /// list
    virtual idx_t getListLength(idx_t headId, idx_t listId) const;

    /// Return the list indices of a particular list back to the CPU
    virtual std::vector<idx_t> getListIndices(idx_t headId, idx_t listId) const;

    /// Return the encoded vectors of a particular list back to the CPU
    virtual std::vector<uint8_t> getListVectorData(idx_t headId, idx_t listId, bool gpuFormat)
            const;
