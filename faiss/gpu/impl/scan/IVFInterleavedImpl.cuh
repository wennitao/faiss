...
template <int THREADS, int NUM_WARP_Q, int NUM_THREAD_Q>
void multiHeadIvfInterleavedScanImpl(
        Tensor<float, 3, true>& queries,
        Tensor<idx_t, 3, true>& listIds,
        DeviceVector<void*>& listData,
        DeviceVector<void*>& listIndices,
    ...
    if (metric_name == MetricType::METRIC_L2) {
        L2Distance metric;
        call_codec(
                multiHead_IVFINT_CODECS<L2Distance, THREADS, NUM_WARP_Q, NUM_THREAD_Q>,
                metric);
    } else if (metric_name == MetricType::METRIC_INNER_PRODUCT) {
        IPDistance metric;
        call_codec(
                multiHead_IVFINT_CODECS<IPDistance, THREADS, NUM_WARP_Q, NUM_THREAD_Q>,
                metric);
    } else {
        FAISS_ASSERT(false);
    }
